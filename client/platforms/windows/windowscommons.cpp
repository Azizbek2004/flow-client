...
constexpr const char *VPN_NAME = "Flow";
constexpr const char *WIREGUARD_DIR = "FlowWG";
...
QString WindowsCommons::tunnelConfigFile()
{
    ...
    for (const QString &path : paths) {
        ...
        logger.debug() << "Found the current wireguard configuration:" << wireguardFile;
    ...
    for (const QString &path : paths) {
        ...
        if (!vpnDir.exists() && !dir.mkdir(VPN_NAME)) {
            logger.debug() << "Failed to create path Flow under" << path;
            continue;
        }
...
{
    logger.debug() << "Getting Current Internet Adapter that routes to" << logger.sensitive(dst.toString());
    ...
    auto adapter = QNetworkInterface::interfaceFromIndex(routeInfo.dwForwardIfIndex);
...
int WindowsCommons::VPNAdapterIndex()
{
    ...
    for (const auto &adapter : adapterList) {
        if (adapter.humanReadableName().contains("Flow")) {
...
bool WindowsCommons::requireSoftwareRendering()
{
    ...
    while (factory->EnumAdapters1(i++, &adapter) != DXGI_ERROR_NOT_FOUND) {
        ...
        result = D3D11CreateDevice(adapter, D3D_DRIVER_TYPE_UNKNOWN, nullptr, 0, nullptr, 0, D3D11_SDK_VERSION, nullptr,
                                   &gpuFeatureLevel, nullptr);
        if (FAILED(result)) {
            logger.error() << "D3D Device" << gpuName << "failed:" << QString::number((quint32)result, 16);
        } else {
            if (gpuFeatureLevel < minFeatureLevel) {
                minFeatureLevel = gpuFeatureLevel;
            }
            logger.debug() << "D3D Device" << gpuName << "supports D3D:" << QString::number(gpuFeatureLevel, 16);
        }
    }
