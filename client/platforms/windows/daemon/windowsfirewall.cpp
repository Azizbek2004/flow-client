...
DEFINE_GUID(ST_FW_WINFW_BASELINE_SUBLAYER_KEY, 0xc78056ff, 0x2bc1, 0x4211, 0xaa, 0xdd, 0x7f, 0x35, 0x8d, 0xef, 0x20,
            0x2d);
...
WindowsFirewall *WindowsFirewall::create(QObject *parent)
{
    ...
    result = FwpmEngineOpen0(nullptr, RPC_C_AUTHN_WINNT, nullptr, &session, &engineHandle);
...
WindowsFirewall::WindowsFirewall(HANDLE session, QObject *parent) : QObject(parent), m_sessionHandle(session)
{
...
bool WindowsFirewall::initSublayer()
{
    ...
    result = FwpmSubLayerGetByKey0(wfp, &ST_FW_WINFW_BASELINE_SUBLAYER_KEY, &maybeLayer);
    if (result == ERROR_SUCCESS) {
    ...
    subLayer.displayData.name = (PWSTR)L"Flow-SplitTunnel-Sublayer";
...
#define FW_OK(rule)                                                                                                    \
    ...
    if (vpnAdapterIndex < 0) {
        ...
        if (!blockTrafficTo(allv4, MED_WEIGHT, "Block Internet", "killswitch")) {
            return false;
        }
        IPAddress allv6("::/0");
        if (!blockTrafficTo(allv6, MED_WEIGHT, "Block Internet", "killswitch")) {
            return false;
        }
    } else
        FW_OK(allowTrafficOfAdapter(vpnAdapterIndex, MED_WEIGHT, "Allow usage of VPN Adapter"));
    FW_OK(allowDHCPTraffic(MED_WEIGHT, "Allow DHCP Traffic"));
    FW_OK(allowHyperVTraffic(MAX_WEIGHT, "Allow Hyper-V Traffic"));
    FW_OK(allowTrafficForAppOnAll(getCurrentPath(), MAX_WEIGHT, "Allow all for Flow.exe"));
...
bool WindowsFirewall::enablePeerTraffic(const InterfaceConfig &config)
{
    ...
    if (!blockTrafficTo(config.m_allowedIPAddressRanges, LOW_WEIGHT, "Block Internet", config.m_serverPublicKey)) {
        return false;
    }
    if (!config.m_primaryDnsServer.isEmpty()) {
        if (!allowTrafficTo(QHostAddress(config.m_primaryDnsServer), 53, HIGH_WEIGHT, "Allow DNS-Server",
        ...
        if (config.m_primaryDnsServer == config.m_serverIpv4Gateway) {
            if (!allowTrafficTo(QHostAddress(config.m_serverIpv6Gateway), 53, HIGH_WEIGHT,
                                "Allow extra IPv6 DNS-Server", config.m_serverPublicKey)) {
                return false;
            }
        }
    }
    ...
    if (!config.m_secondaryDnsServer.isEmpty()) {
        if (!allowTrafficTo(QHostAddress(config.m_secondaryDnsServer), 53, HIGH_WEIGHT, "Allow DNS-Server",
                            config.m_serverPublicKey)) {
            return false;
        }
        // In some cases, we might configure a 2nd DNS server for IPv6, however
    ...
    for (const QString &dns : config.m_allowedDnsServers) {
        ...
        if (!allowTrafficTo(QHostAddress(dns), 53, HIGH_WEIGHT, "Allow DNS-Server", config.m_serverPublicKey)) {
            return false;
        }
    }
    ...
    if (!config.m_excludedAddresses.empty()) {
        for (const QString &i : config.m_excludedAddresses) {
            ...
            if (!allowTrafficTo(i, HIGH_WEIGHT, "Allow Ecxlude route", config.m_serverPublicKey)) {
                return false;
            }
        }
    }
...
bool WindowsFirewall::allowTrafficForAppOnAll(const QString &exePath, int weight, const QString &title)
{
    DWORD result = ERROR_SUCCESS;
    Q_ASSERT(weight <= 15);
...
bool WindowsFirewall::allowTrafficOfAdapter(int networkAdapter, uint8_t weight, const QString &title)
{
    FWPM_FILTER_CONDITION0 conds;
    ...
    // #1 Permit outbound IPv4 traffic.
    filter.layerKey = FWPM_LAYER_ALE_AUTH_CONNECT_V4;
    if (!enableFilter(&filter, title, description.arg("out").arg(networkAdapter))) {
        return false;
    }
    // #2 Permit inbound IPv4 traffic.
    filter.layerKey = FWPM_LAYER_ALE_AUTH_RECV_ACCEPT_V4;
    if (!enableFilter(&filter, title, description.arg("in").arg(networkAdapter))) {
        return false;
    }
...
bool WindowsFirewall::allowTrafficTo(const IPAddress &addr, int weight, const QString &title, const QString &peer)
{
    GUID layerKeyOut;
    GUID layerKeyIn;
...
bool WindowsFirewall::allowTrafficTo(const QHostAddress &targetIP, uint port, int weight, const QString &title,
                                     const QString &peer)
{
    ...
    GUID layerIn = isIPv4 ? FWPM_LAYER_ALE_AUTH_RECV_ACCEPT_V4 : FWPM_LAYER_ALE_AUTH_RECV_ACCEPT_V6;
    ...
    if (!enableFilter(&filter, title, description.arg("to").arg(targetIP.toString()).arg(port), peer)) {
        return false;
    }
    filter.layerKey = layerIn;
    if (!enableFilter(&filter, title, description.arg("from").arg(targetIP.toString()).arg(port), peer)) {
        return false;
    }
    return true;
}
...
bool WindowsFirewall::blockTrafficTo(const IPAddress &addr, uint8_t weight, const QString &title, const QString &peer)
{
    ...
    filter.layerKey = layerKeyOut;
    if (!enableFilter(&filter, title, description.arg("to").arg(addr.toString()), peer)) {
        return false;
    }
    filter.layerKey = layerKeyIn;
    if (!enableFilter(&filter, title, description.arg("from").arg(addr.toString()), peer)) {
        return false;
    }
    return true;
}

bool WindowsFirewall::blockTrafficTo(const QList<IPAddress> &rangeList, uint8_t weight, const QString &title,
                                     const QString &peer)
{
...
void WindowsFirewall::importAddress(const QHostAddress &addr, OUT FWP_VALUE0_ &value, OUT QByteArray *v6DataBuffer)
{
...
bool WindowsFirewall::blockTrafficOnPort(uint port, uint8_t weight, const QString &title)
{
    // Allow Traffic to IP with PORT using any protocol
    FWPM_FILTER_CONDITION0 conds[3];
    conds[0].fieldKey = FWPM_CONDITION_IP_PROTOCOL;
...
bool WindowsFirewall::enableFilter(FWPM_FILTER0 *filter, const QString &title, const QString &description,
                                   const QString &peer)
{
    uint64_t filterID = 0;
...
bool WindowsFirewall::allowLoopbackTraffic(uint8_t weight, const QString &title)
{
    ...
    for (const auto &iface : networkInterfaces) {
        ...
        if (!allowTrafficOfAdapter(iface.index(), weight, title.arg(iface.name()))) {
            return false;
        }
    }
    return true;
