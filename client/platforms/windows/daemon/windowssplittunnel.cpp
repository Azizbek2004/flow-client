...
constexpr static const auto DRIVER_SERVICE_NAME = L"FlowSplitTunnel";
...
std::unique_ptr<WindowsSplitTunnel> WindowsSplitTunnel::create(WindowsFirewall *fw)
{
    ...
    if (Q_UNLIKELY(driver_manager == nullptr)) {
        ...
        Q_ASSERT(false);
        logger.error() << "WindowsServiceManager was unable fo find Split Tunnel service?";
        return nullptr;
    }
    ...
    if (driverFile == INVALID_HANDLE_VALUE) {
        ...
        logger.info() << "Failed to open driver, attempting only once to reboot driver";
...
bool WindowsSplitTunnel::initDriver(HANDLE driverIO)
{
    ...
    DWORD bytesReturned;
    auto ok = DeviceIoControl(driverIO, IOCTL_INITIALIZE, nullptr, 0, nullptr, 0, &bytesReturned, nullptr);
    if (!ok) {
...
bool WindowsSplitTunnel::excludeApps(const QStringList &appPaths)
{
    ...
    DWORD bytesReturned;
    auto ok = DeviceIoControl(m_driver, IOCTL_SET_CONFIGURATION, &config[0], (DWORD)config.size(), nullptr, 0,
...
bool WindowsSplitTunnel::start(int inetAdapterIndex, int vpnAdapterIndex)
{
    ...
    if (getState() == STATE_INITIALIZED) {
        ...
        auto ok = DeviceIoControl(m_driver, IOCTL_REGISTER_PROCESSES, &config[0], (DWORD)config.size(), nullptr, 0,
                                  &bytesReturned, nullptr);
        if (!ok) {
    ...
    auto ok = DeviceIoControl(m_driver, IOCTL_REGISTER_IP_ADDRESSES, &config[0], (DWORD)config.size(), nullptr, 0,
                              &bytesReturned, nullptr);
    if (!ok) {
...
{
    DWORD bytesReturned;
    auto ok = DeviceIoControl(m_driver, IOCTL_CLEAR_CONFIGURATION, nullptr, 0, nullptr, 0, &bytesReturned, nullptr);
    if (!ok) {
...
{
    DWORD bytesReturned;
    auto ok = DeviceIoControl(driverIO, IOCTL_ST_RESET, nullptr, 0, nullptr, 0, &bytesReturned, nullptr);
    if (!ok) {
...
WindowsSplitTunnel::DRIVER_STATE WindowsSplitTunnel::getState(HANDLE driverIO)
{
    ...
    DWORD bytesReturned;
    SIZE_T outBuffer;
    bool ok = DeviceIoControl(driverIO, IOCTL_GET_STATE, nullptr, 0, &outBuffer, sizeof(outBuffer), &bytesReturned,
                              nullptr);
    if (!ok) {
...
std::vector<uint8_t> WindowsSplitTunnel::generateAppConfiguration(const QStringList &appPaths)
{
    ...
    size_t bufferSize =
            sizeof(CONFIGURATION_HEADER) + (sizeof(CONFIGURATION_ENTRY) * appPaths.size()) + cummulated_string_size;
...
std::vector<std::byte> WindowsSplitTunnel::generateIPConfiguration(int inetAdapterIndex, int vpnAdapterIndex)
{
    ...
    // Always the VPN
    if (!getAddress(vpnAdapterIndex, &config->TunnelIpv4, &config->TunnelIpv6)) {
        return {};
    }
    // 2nd best route is usually the internet adapter
    if (!getAddress(inetAdapterIndex, &config->InternetIpv4, &config->InternetIpv6)) {
        return {};
    };
    return out;
}
...
{
    LPCWSTR displayName = L"Flow Split Tunnel Service";
    QFileInfo driver(qApp->applicationDirPath() + "/" + DRIVER_FILENAME);
    if (!driver.exists()) {
        logger.error() << "Split Tunnel Driver File not found " << driver.absoluteFilePath();
    ...
    auto serviceManager = OpenSCManager(nullptr, // local computer
                                        ...
                                        scm_rights);
    auto service = CreateService(serviceManager, DRIVER_SERVICE_NAME, displayName, SERVICE_ALL_ACCESS,
                                 SERVICE_KERNEL_DRIVER, SERVICE_AUTO_START, SERVICE_ERROR_NORMAL, binPath, nullptr,
...
bool WindowsSplitTunnel::isRunning()
{
...
QString WindowsSplitTunnel::stateString()
{
    ...
    case STATE_ZOMBIE: return "STATE_ZOMBIE"; break;
    }
    return {};
}
