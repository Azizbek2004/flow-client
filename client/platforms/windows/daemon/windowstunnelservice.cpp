...
#define TUNNEL_NAMED_PIPE                                                                                              \
    "\\\\."                                                                                                            \
    "\\pipe\\ProtectedPrefix\\Administrators\\FlowWG\\Flow"
...
WindowsTunnelService::WindowsTunnelService(QObject *parent) : QObject(parent)
{
    ...
    SC_HANDLE service = OpenService((SC_HANDLE)m_scm, TUNNEL_SERVICE_NAME, SERVICE_ALL_ACCESS);
    if (service != nullptr) {
...
bool WindowsTunnelService::start(const QString &configData)
{
    ...
    QString serviceCmdline;
    {
        ...
        out << "\"" << qApp->applicationFilePath() << "\" tunneldaemon \"" << configData << "\"";
    }

    logger.debug() << "Service:" << qApp->applicationFilePath();

    service = CreateService(scm, TUNNEL_SERVICE_NAME, L"Flow (tunnel)", SERVICE_ALL_ACCESS, SERVICE_WIN32_OWN_PROCESS,
                            SERVICE_DEMAND_START, SERVICE_ERROR_NORMAL, (const wchar_t *)serviceCmdline.utf16(),
                            nullptr, 0, TEXT("Nsi\0TcpIp\0"), nullptr, nullptr);
    if (!service) {
        WindowsUtils::windowsLog("Failed to create the tunnel service");
        return false;
    }

    SERVICE_DESCRIPTION sd = { (wchar_t *)L"Manages the Flow tunnel connection" };

    if (!ChangeServiceConfig2(service, SERVICE_CONFIG_DESCRIPTION, &sd)) {
        WindowsUtils::windowsLog("Failed to set the description to the tunnel service");
        return false;
    }
    ...
    logger.debug() << "The tunnel service exited with status:" << status.dwWin32ExitCode << "-"
                   << exitCodeToFailure(&status);
...
static bool stopAndDeleteTunnelService(SC_HANDLE service)
{
    ...
    logger.debug() << "The current service is stopped:" << (status.dwCurrentState == SERVICE_STOPPED);
...
QString WindowsTunnelService::uapiCommand(const QString &command)
{
    ...
    HANDLE pipe = CreateFile(tunnelName, GENERIC_READ | GENERIC_WRITE, 0, nullptr, OPEN_EXISTING, 0, nullptr);
...
QString WindowsTunnelService::exitCodeToFailure(const void *status)
{
    ...
    case 1: return "Error when opening the ringlogger log file";
    case 2:
        return "Error while loading the WireGuard configuration file from "
               "path.";
    case 3: return "Error while creating a WinTun device.";
    case 4: return "Error while listening on a named pipe.";
    case 5: return "Error while resolving DNS hostname endpoints.";
    case 6: return "Error while manipulating firewall rules.";
    case 7: return "Error while setting the device configuration.";
    case 8: return "Error while binding sockets to default routes.";
    case 9:
        return "Unable to set interface addresses, routes, dns, and/or "
               "interface settings.";
    case 10: return "Error while determining current executable path.";
    case 11: return "Error while opening the NUL file.";
    case 12: return "Error while attempting to track tunnels.";
    case 13: return "Error while attempting to enumerate current sessions.";
    case 14: return "Error while dropping privileges.";
    case 15: return "Windows internal error.";
    default: return QString("Unknown error (%1)").arg(st->dwServiceSpecificExitCode);
    }
}
